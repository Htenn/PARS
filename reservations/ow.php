            ...
            <section id="content" class="main">
                <?php
                    if (isset($_GET['view'])) {
                        ...
                        $query = "SELECT *, flightNumber as flight FROM booking WHERE bookingID = $ID";
                        $queryResult = mysqli_query($db, $query);

                        if ($display = mysqli_fetch_array($queryResult)) {
                            echo "
                                <div class='col-6 col-12-xsmall'>
                                    <h3><strong>Flight Number: </strong>" . $display['flight'] . "</h3>
                                </div>
                                ";
                            echo "
                                    <div class='row'>
                                        <div class='col-6 col-12-xsmall'>
                                            <h3><strong>Origin: </strong>" . $display['Origin'] . "</h3>
                                        </div>
                                        <div class='col-6 col-12-xsmall'>
                                            <h3><strong>Destination: </strong>" . $display['Destination'] . "</h3>
                                        </div>
                                    </div>
                                ";
                            
                            $flightInfo = "SELECT * FROM flight WHERE flightNumber = '" . $display['flight'] . "' LIMIT 1";
                            ...
                            $seatsQuery = "SELECT * FROM flight_seat WHERE (clientID = " . $display['clientID'] . " AND flightNumber = '" . $display['flight'] . "' AND bookingID = $ID)";
                            ...
                            while ($flight_seatTable = mysqli_fetch_array($seatsResult)) {
                                ...
                                $pnrFlightInfoQuery = "SELECT * FROM pnr_flight WHERE (ID = " . $flight_seatTable['passengerID'] . " AND flightNumber = '" . $display['flight'] . "')";
